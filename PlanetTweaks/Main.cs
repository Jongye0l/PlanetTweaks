    ...
    private static void OnGUI(UnityModManager.ModEntry modEntry) {
        ...
        if(ADOBase.hasTaroDLC && ADOBase.ownsTaroDLC) {
            ...
            Settings.thirdColorType = GUILayout.Toolbar(Settings.thirdColorType,
                RDString.language == SystemLanguage.Korean
                    ? new string[] {
                        $"<color={ColorUtils.GetRealDefaultThirdColor().Hex()}>기본</color>", $"<color={ColorUtils.GetRealColor(true).Hex()}>불 행성</color>", $"<color={ColorUtils.GetRealColor(false).Hex()}>얼음 행성</color>", $"<color={ColorUtils.GetCustomThirdColor().Hex()}>커스텀</color>"
                    }
                    : new string[] {
                        $"<color={ColorUtils.GetRealDefaultThirdColor().Hex()}>Default</color>", $"<color={ColorUtils.GetRealColor(true).Hex()}>Fire Planet</color>", $"<color={ColorUtils.GetRealColor(false).Hex()}>Ice Planet</color>",
                        $"<color={ColorUtils.GetCustomThirdColor().Hex()}>Custom</color>"
                    });
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
            GUILayout.Space(5);
